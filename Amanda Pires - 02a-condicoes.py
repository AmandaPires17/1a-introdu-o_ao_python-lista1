...
def data_valida(data):
    ...
    return (
        ano >= 1 and (mes >= 1 and mes <= 12)
        and (dia >= 1 and dia <= maior_dia_do_mes(mes,ano))
    )
...
def baskara(a, b, c):
    ...
    if a != 0:
        ...
        if _delta < 0:
            ...
            raiz1 = (-b + sqrt(_delta)) / (2 * a)
            raiz2 = (-b - sqrt(_delta)) / (2 * a)
            return(raiz1, raiz2)
    else:
        return (-c / b,)
...
def acrescimo_nota_bb(nota_sozinho, nota_com_ajuda):
    ...
    """
    melhoria = nota_com_ajuda - nota_sozinho
    if melhoria > 0:
        acrecimo = melhoria / 4
        return round(acrecimo,1)
    else:
        return 0
